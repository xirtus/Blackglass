...
test.describe('authentic atlas map experience', () => {
  test('renders real city, room, and world atlas surfaces', async ({ page }) => {
    for (const viewport of [
      ...
      expect(cityStats.firstTileSrc).toContain('tile.openstreetmap.org')
      ...
      expect(worldStats.firstTileSrc).toContain('tile.openstreetmap.org')
    }
  })
  ...
  test('CCTV and viewshed layers can be toggled like an OSINT dashboard', async ({ page }) => {
    ...
    await expect(page.getByTestId('camera-source-link')).toHaveAttribute('href', /data\.gov|dc\.gov/)
    ...
    await expect(page.getByTestId('camera-feed-view')).toContainText(cameraName)
    expect(await page.getByTestId('camera-feed-view').textContent()).not.toBe(firstCameraLabel)
    ...
    await expect(page.getByTestId('camera-feed-view')).toContainText(listCameraLabel ?? '')
  ...
  test('public camera feeds render real media when the source exposes it', async ({ page }) => {
    ...
    await expect(page.getByTestId('camera-feed-view')).toContainText('REAL HLS')
  })
  ...
  test('city map exposes direct cursor rotation and pan affordances', async ({ page }) => {
    ...
    await page.mouse.move((box?.x ?? 0) + 520, (box?.y ?? 0) + 300)
    await page.mouse.down()
    await page.mouse.move((box?.x ?? 0) + 760, (box?.y ?? 0) + 300)
    await page.mouse.up()
